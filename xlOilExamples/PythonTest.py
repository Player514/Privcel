...
def priv_help(func = ""):
    if(func == "Priv_check"):
        return ["header: np.array,data: np.array,distribution: np.array,request :String,observations: String)",
        "Runs an analysis of leakage created from running request on data",
        "=priv_check(A114:D114,A115:D139,A140:D140)"]
    if(func == ""):
        return ["Use priv_help(\"function\") to check function",
        "=priv_help(\"priv_check\")"]
...
@xlo.func    
def gen_data(x=5,y=5, min=0, max=10,header = 'Score,Age,Salary,Height'):
    if(header or header != ''):
        length = len(header.split(','))
        return np.append([header.split(',')],np.random.rand(int(x),int(length))*(max-min)-min,axis =0)
    return np.random.rand(int(x),int(y))*(max-min)-min

@xlo.func    
def gen_data_help1():
    return 'x=5,y=5, min=0, max=10,header = Name,Age,Gender'
#switch
# ages = pv.Normal('ages',mu=35,std=2,num_elements=100)
#ages = pv.bernulli('ages',mu=35,std=2,num_elements=100)
# 

@xlo.func
def CreateHyperperameters(x=5,y=5, min=0, max=10):
    #normal, binomial, etc.
    explain = ['Type', 'Mean','Variance']
    row1 = ['Normal', 5,5]
    row2 = ['Binomial', 35,5]
    return np.array([explain,row1,row2])

@xlo.func(macro=True)
def pyTestPlot(width=100, height=100):
...
def pyTestPic():
    ...
    im = Image.open('c:\\Users\\andre\\office\\privbugger\\figure1.png')
#    im2 = Image.open('c:\\Users\\andre\\office\\privbugger\\figure_Name.png')
    return [im]
...
def show_pic(ID):
    ...
    im = Image.open('c:\\Users\\andre\\office\\privbugger\\' + ID + '.png')
#    im2 = Image.open('c:\\Users\\andre\\office\\privbugger\\figure_Name.png')
    return [im]
...
def pyTestPic2():
    ...
    im = Image.open('c:\\Users\\andre\\office\\privbugger\\figure_Name.png')
    return im
...
@xlo.func
def pyTestWorkbooks():
    return  [x.name for x in xlo.workbooks] #[xlo.workbooks.active.name] +

@xlo.func
def pyTestWorkbooks2():
    
    return str(type([xlo.workbooks.active.name] + [x.name for x in xlo.workbooks]))
...
try:
    ...
    @xlo.func
    def sum_data(arr: np.array, ax = 1) -> np.array:
        #r_h = xlo.Range(range)
        arr = arr.astype(float)
        if(ax == 0):
            sum = [arr.sum(axis = int(ax))]
        else:
            sum = arr.sum(axis = int(ax))
        return sum
        #df: pd.DataFrame) -> xlo.Cache
    #@xlo.func
    #async def pyRtdArray(values):
    #return np.sum(values)

    @xlo.func
    def priv_check(header: np.array,data: np.array,distribution: np.array,request = "sum",observations = "") -> np.array:
        dictionary = {
        ...
        #TODO set ID equal to hash or request
        #ID += 1
        #serialize
        json_object = json.dumps(dictionary, indent=4)
        #write
        with open("C:/Users/andre/office/sample.json", "w") as outfile:
        ...
        if "png" in response:
            ...
            return ['Refresh to Update','Running Request',response]

    def read_response(ID) -> np.array:
        with open("C:/Users/andre/office/response.json", "r") as outfile:
            response = json.load()
    ...
    @xlo.func
    def priv_analyze(idx: xlo.Range):
        return str(idx.formula)
    ...
    @xlo.func(args={'df': "Data to be read as a pandas dataframe"})
    def pytry():
        from openpyxl.styles import Font #TintAndShade
#        xlo.app().range("F113").AutoOutline()
#        return xlo.app().range("F113").FormatConditions.AddColorScale(1) #2071986176*2
#        xlo.app().range("F113").ColorFormat = 5
        xlo.app().range("F113").Font.Color = 4228250625 #2071986176*2
#        return str(xlo.app().range("F113").Font.Color)
        #df: pd.DataFrame) -> xlo.Cache
    #@xlo.func
    #async def pyRtdArray(values):
    #return np.sum(values)
    ...
    def pyTestDFrameIndex(df: PDFrame(headings=True, index="Time")):
        return xlo.cache(df) 
    @xlo.func
    def pyDFrameIndex(df: PDFrame(headings=False)):
        return xlo.cache(df) 
